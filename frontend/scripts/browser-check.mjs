...
assert.equal(await page.getByText('PHOTOSENSITIVITY / SEIZURE WARNING',{exact:true}).count(),1);
assert.equal(await page.getByText('DO NOT TURN UP THE VOLUME TO 90 OR MORE IF YOU HAVE EPILEPSY.',{exact:true}).count(),1);
await page.getByRole('button',{name:'YES I AM EPILEPTIC',exact:true}).click();
assert.equal(await page.locator('#music-volume').getAttribute('max'),'89');
assert.equal(await page.getByRole('button',{name:'DISCO MODE DISABLED',exact:true}).count(),1);
await page.locator('#music-volume').evaluate(element=>{element.value='100';element.dispatchEvent(new Event('input',{bubbles:true}));});
await page.waitForTimeout(100);
assert.equal(await page.evaluate(()=>window.portfolioDebug.bugHunt.partyMode),false);

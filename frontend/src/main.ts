...
import { warmBackend } from './services';
...
document.querySelector('.welcome-warning')!.insertAdjacentHTML('afterend','<p class="music-attribution">Background music from: <a href="https://www.youtube.com/watch?v=PEqjXrE6YgM" rel="noreferrer" target="_blank">https://www.youtube.com/watch?v=PEqjXrE6YgM</a></p>');
...
for(const [i,landmark] of landmarks.entries()){
  ...
  const element=document.createElement('div');element.className='landmark-label';element.innerHTML=`<span>${String(i+1).padStart(2,'0')}</span><div><strong>${landmark.title}</strong></div>`;

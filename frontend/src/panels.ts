...
export async function renderContent(element:HTMLElement, definition:PanelDefinition) {
  ...
  if(definition.type==='iframe') {
    ...
    } catch { element.textContent='This embedded page is not available.'; }
  }
}
...
export class PanelSystem {
  ...
  constructor(scene:THREE.Scene) {
    for(const landmark of landmarks){
      ...
      for(const [index,id] of [landmark.frontPanel,landmark.backPanel].entries()){
        ...
        this.service.get(id).then(def=>renderContent(el,def)).catch(()=>{el.textContent='This notebook could not be opened. Approach again or reload to retry.';});
      }
  ...
  update(player:THREE.Vector3,camera:THREE.PerspectiveCamera,dt:number) {
    ...
    for(const s of this.surfaces){
      ...
      s.elements.forEach((el,index)=>{const visible=p>0&&(index===0?front:!front);el.style.visibility=visible?'visible':'hidden';el.style.pointerEvents=visible&&p>.8?'auto':'none';el.inert=!visible;el.style.opacity=String(Math.min(1,p*3));el.dataset.state=s.state;});
    }
  }
}
